...
# def checkModels():
#     models, packs = getModelsAndPacks()
#     modelsToInstall = []
#     p = open("spack-info.txt")
#     for g in packs:
#         if g not in p.read():
#             mod = g.split('@')
#             mod = mod[0].upper()+'-'+mod[1]
#             modelsToInstall.append(mod)
#     return modelsToInstall

# def moveModels(modelsToInstall):
#     if modelsToInstall:
#         print("New Machine with Missing Models Detected!")
#         for m in modelsToInstall:
#             os.system("cp ~/agave-model/models/%s/* ~/agave-model/models_to_install/" % m.upper())
...
#                     #For QBC
#                     print("singularity exec $SING_OPTS --pwd $PWD $IMAGE bash ./tarballs.sh", file=fd)
#                     print("singularity exec $SING_OPTS --pwd $PWD $IMAGE bash -l ./build-models.sh", file=fd)
...
#         os.system("chmod 755 run_dir/get-versions.sh")
#         with open("run_dir/runapp.sh","w") as fd:
#             print("singularity exec $SING_OPTS --pwd $PWD $IMAGE bash -l ./get-versions.sh", file=fd)
#         os.system("tar czvf input.tgz run_dir")
